...
//events sent: connected, connectionLost, configReceived, alarm, alarmImages, analysisState
...
function gotState(err, data) {
    if (err) {
        this.AnalysisState = null;
    } else {
        this.AnalysisState = _.map(data.camera ? data.camera : [], function(cam) {
            return {
                name: cam.$.name,
                id: cam.$.camid,
                state: cam.$.signal == "on" && cam.$.analysis == "on"
            };
        })
    }

    this.emit('analysisState', this.AnalysisState);
}

function periodic_test() {
    if (this.ConnectionEstablished) //send ping
    {
        this.client.send({ "$": { type: "state" } }, gotState.bind(this));
    } else //reconnect
    {
        connectClient.call(this);
    }
...
function enableAnalysis(bState) {
    if (this.ConnectionEstablished) //send ping
    {
        this.client.send({ "$": { type: "setevent", name: "AnalyseAll", value: bState ? "1" : "0" } }, periodic_test.bind(this));
    }
}
